<?php
  ...
  function htmlParser($getData, $mode) {
    ...
    else if($mode == 'getDataTransferResponse') {<?php
  // Steve OCPP HTTP client emulator
    ...
    switch ($getData['cmd']) {
      ...
      case 'RemoteStartTransaction':
        ...
        if($allow) {
          ...
          $token = getCSRFToken($content);
          // Prepare form
          $form = "chargePointSelectList=JSON;".$getData['ChargeBoxID'].";-&connectorId=".$getData['ConnectorID']."&idTag=".$getData['idTag']."&_csrf=".$token."";
          // Send form
          curl_setopt($curl, CURLOPT_POSTFIELDS, $form);
          curl_exec($curl);
        }
      ...
      case 'RemoteStopTransaction':
        ...
        if($allow) {
          ...
          $token = getCSRFToken($content);
          // Prepare form
          $form = "chargePointSelectList=JSON;".$getData['ChargeBoxID'].";-&transactionId=".$steveTransactionID."&_csrf=".$token."";
          // Send form
          curl_setopt($curl, CURLOPT_POSTFIELDS, $form);
          curl_exec($curl);
        }
      ...
      case 'UnlockConnector':
        ...
        if($allow) {
          ...
          $token = getCSRFToken($content);
          // Prepare form
          $form = "chargePointSelectList=JSON;".$getData['ChargeBoxID'].";-&connectorId=".$getData['ConnectorID']."&_csrf=".$token."";
          // Send form
          curl_setopt($curl, CURLOPT_POSTFIELDS, $form);
          curl_exec($curl);
        }
      ...
      case 'DataTransfer':
        ...
        if($allow) {
          ...
          $token = getCSRFToken($content);
          // Prepare form
          $form = "chargePointSelectList=JSON;".$getData['ChargeBoxID'].";-&vendorId=".$getData['VendorID']."&messageId=".$getData['MessageID']."&data=".$getData['Data']."&_csrf=".$token."";
          // Send form
          curl_setopt($curl, CURLOPT_POSTFIELDS, $form);
          curl_exec($curl);
          // Sleep.. (Wait response)
